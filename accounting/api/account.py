...
@account_router.get("/get_all", response=List[AccountOut], tags = ['others'])
def get_all(request):
...
@account_router.get('/get_account_types/', tags = ['others'])
def get_account_types(request):
    return {t[0]: t[1] for t in AccountTypeChoices.choices}


@account_router.get('/account-balance/{account_id}', response=GeneralLedgerOut, tags = ['others'])
...
@account_router.get('/account-balances/', response=List[GeneralLedgerOut], tags = ['others'])
...
@account_router.get('/total-account-balances/', response=List[TotalBalance], tags = ['Task 3 End Points'])
...
@account_router.get('/total-account-balance/', response = {200: TotalBalance, 404: FourOFourOut}, tags = ['Task 3 End Points'])
